...
public class BTreeInternalPage extends BTreePage {
	...
	public int getMaxEntries() {
		...
		// one extra child pointer (node with m entries has m+1 pointers to children), 1
		// bit for extra header
	...
	/**
	 * Computes the number of bytes in the header of a B+ internal page with each
	 * entry occupying entrySize bytes
	 * 
	 ...
	 * recordId
	 * is used to find the specified entry, so it must not be null. After deletion,
	 * the
	 * entry's recordId should be set to null to reflect that it is no longer stored
	 * on
	 * any page.
	 * 
	...
	/**
	 * Update the key and/or child pointers of an entry at the location specified by
	 * its
	 * record id.
	 * 
	 ...
	 *                     already empty, or updating this key would put the entry
	 *                     out of
	 *                     order on the page
	 */
	...
	/**
	 * Adds the specified entry to the page; the entry's recordId should be updated
	 * to
	 * reflect that it is now stored on this page.
	 * 
	 * @throws DbException if the page is full (no empty slots) or key field type,
	 *                     table id, or child page category is a mismatch, or the
	 *                     entry is invalid
		...
		for (int i = 0; i < numSlots; i++) {
			if (isSlotUsed(i)) {
				if (children[i] == e.getLeftChild().pageNumber() || children[i] == e.getRightChild().pageNumber()) {
					...
					// validate that the next key is greater than or equal to the one we are
					// inserting
					if (keys[i].compare(Op.LESS_THAN, e.getKey())) {
						throw new DbException("attempt to insert invalid entry with left child " +
								e.getLeftChild().pageNumber() + ", right child " +
	...
	/**
	 * @return a reverse iterator over all entries on this page (calling remove on
