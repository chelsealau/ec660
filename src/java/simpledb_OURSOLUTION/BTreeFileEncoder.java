...
public class BTreeFileEncoder {
	 ...
	 * 
	 * @param tuples         - list of tuples to add to the file
	 * @param hFile          - the file to temporarily store the data as a heap file
	 *                       on disk
	 * @param bFile          - the file on disk to back the resulting BTreeFile
	...
	public static BTreeFile convert(File inFile, File hFile, File bFile, int npagebytes,
		...
		// We wait until we have two full pages of tuples before writing out the first
		// page
		// so that we will not end up with any pages containing less than nrecords/2
		// tuples
		...
		// 1. We have less than or equal to a full page of records. Because of the way
		// the code
		...
		// For case (2), we divide the remaining records equally between the last two
		// pages,
	 ...
	 * @param pid            - the id of the page to update with the right sibling
	 *                       pointer
	...
	private static void cleanUpEntries(ArrayList<ArrayList<BTreeEntry>> entries,
		...
		// 1. We have less than or equal to a full page of entries. Because of the way
		// the code
		// was written, we know this must be the root page
		// 2. We have somewhere between one and two pages of entries remaining.
		// For case (1), we write out the page
		// For case (2), we divide the remaining entries equally between the last two
		// pages,
		// write them out, and update the parent's child pointers.
	 ...
	 * @param npagebytes        - number of bytes per page
	 * @param keyType           - the type of the key field
	 * @param childPageCategory - the category of the child pages (either internal
	 *                          or leaf)
