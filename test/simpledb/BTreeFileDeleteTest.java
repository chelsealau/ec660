...
public class BTreeFileDeleteTest extends SimpleDbTestBase {
	...
	public void testStealFromLeftLeafPage() throws Exception {
		...
		BTreeLeafPage sibling = BTreeUtility.createRandomLeafPage(siblingId, 2, keyField, 0,
				BTreeUtility.MAX_RAND_VALUE / 2);
	...
	public void testStealFromRightLeafPage() throws Exception {
		...
		BTreeLeafPage sibling = BTreeUtility.createRandomLeafPage(siblingId, 2, keyField, BTreeUtility.MAX_RAND_VALUE / 2,
				BTreeUtility.MAX_RAND_VALUE);
	...
	public void testStealFromLeftInternalPage() throws Exception {
		...
		assertTrue(
				parent.iterator().next().getKey().compare(Op.GREATER_THAN_OR_EQ, sibling.reverseIterator().next().getKey()));
	...
	public void testStealFromRightInternalPage() throws Exception {
		...
		assertTrue(
				parent.iterator().next().getKey().compare(Op.GREATER_THAN_OR_EQ, page.reverseIterator().next().getKey()));
	...
	public void testMergeInternalPages() throws Exception {
		...
		BTreeInternalPage rightPage = BTreeUtility.createRandomInternalPage(rightPageId, keyField, BTreePageId.LEAF,
				entriesPerPage / 2 - 1, BTreeUtility.MAX_RAND_VALUE / 2, BTreeUtility.MAX_RAND_VALUE,
				2 + 3 * entriesPerPage / 2);
